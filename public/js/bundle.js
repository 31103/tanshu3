 ...
 */
function formatDate(dateStr, format = 'yyyymmdd') {
  // 00000000の場合はそのまま返す
...
function formatResults(
  cases,
  settings = { showAllCases: false, dateFormat: 'yyyymmdd' },
 ...
 * @returns 検証結果オブジェクト
 */
function validateEFFile(content) {
  // 初期の検証結果
  const result = {
    isValid: true,
    errors: [],
  ...
  for (let i = 1; i <= sampleSize; i++) {
    ...
    if (columns.length < 30) {
      ...
      if (!columnsCountWarning) {
        result.warnings.push(
          `一部の行に必要な列数（30列以上）がありません。（最初の例：行 ${i + 1}, 列数: ${columns.length}）`,
        );
    ...
    if (!/^\d{9}$/.test(facilityCode)) {
      ...
      if (!result.warnings.some((w) => w.includes('施設コード'))) {
        result.warnings.push(
          `一部の行の施設コードが適切なフォーマット（9桁の数字）ではありません。（最初の例：行 ${i + 1}, 値: ${facilityCode}）`,
        );
      }
    }
    ...
    if (!dataId || dataId.length > 10) {
      ...
      if (!result.warnings.some((w) => w.includes('データ識別番号'))) {
        result.warnings.push(
          `一部の行のデータ識別番号が不適切です（空または10桁超）。（最初の例：行 ${i + 1}, 値: ${dataId}）`,
        );
      }
    }
    ...
    if (!dateRegex.test(discharge)) {
      ...
      if (!result.warnings.some((w) => w.includes('退院年月日'))) {
        result.warnings.push(
          `一部の行の退院年月日が適切なフォーマット（8桁の数字または00000000）ではありません。（最初の例：行 ${i + 1}, 値: ${discharge}）`,
        );
      }
    }
    if (!dateRegex.test(admission)) {
      ...
      if (!result.warnings.some((w) => w.includes('入院年月日'))) {
        result.warnings.push(
          `一部の行の入院年月日が適切なフォーマット（8桁の数字または00000000）ではありません。（最初の例：行 ${i + 1}, 値: ${admission}）`,
        );
      }
    }
    ...
    if (!/^\d{1,2}$/.test(dataCategory)) {
      ...
      if (!result.warnings.some((w) => w.includes('データ区分'))) {
        result.warnings.push(
          `一部の行のデータ区分が適切なフォーマット（2桁以内の数字）ではありません。（最初の例：行 ${i + 1}, 値: ${dataCategory}）`,
        );
      }
    }
    // 有効な行としてカウント
  ...
  if (validLineCount === 0 && sampleSize > 0) {
    result.isValid = false;
    result.errors.push(
      'サンプリングした行の中に有効なデータ行がありませんでした。ファイルフォーマットが入院統合EFファイルの仕様に合っていない可能性があります。',
    );
  } else if (invalidLineCount > validLineCount) {
    result.warnings.push(
      '無効なデータ行が有効な行より多く見つかりました。ファイルの内容を確認してください。',
    );
  }
  return result;
}
