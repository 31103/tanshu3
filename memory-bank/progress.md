...
- **全体進捗:** **プロジェクト全体の整理完了。短手３判定ロジック修正（加算除外）完了。CI/CD 推進計画 (Issue #1) 完了。Deno 移行完了。**
- **直近のマイルストーン:** プロジェクト全体の整理。
- **現在のタスク:** **整理内容のコミットと Pull Request 作成**

## 2. 完了した機能 (What Works)

- **プロジェクト整理:**
  - 不要なレガシーファイル (`src/core/common.ts`) を削除。
  - コメントアウトされていた不要な `console.warn` を削除 (`src/ui/components/notification.ts`, `src/core/common/parsers.ts`)。
  - パーサーの TODO コメント（スキップ行のユーザー通知）に関する Issue #8 を作成。
  - Lint/Format チェック、テスト実行により変更によるリグレッションがないことを確認。
...
- **短手３判定 (`evaluator.ts`):**
  ...
  - **他の手術の判定基準を「データ区分 '50' かつコード '15' 始まり、かつ診療明細名称に『加算』を含まない」に修正。**
...
- 関連テスト修正・追加。
- **短手３判定ロジック修正（加算除外）:**
  - `evaluator.ts` の「他の手術」判定ロジックを修正し、「加算」を含む診療明細名称を持つものを除外。
  - `evaluator_test.ts` に関連テストケースを追加。

## 3. 残りの作業 (What's Left to Build)

- **[未着手] CI/CD ワークフローテスト:** (PR マージ後 or 別途実施)
  - **CI:** `main` ブランチへのマージ時に `ci.yml` が正常に動作するか確認。
...
- **[保留] パーサー: スキップ行のユーザー通知機能実装 (Issue #8):** `NotificationSystem` を使用してユーザーに通知する機能。
...
- **[2025-04-11]:** プロジェクト全体の整理を実施。不要なレガシーファイル (`src/core/common.ts`) とコメントアウトされたデバッグコードを削除。パーサーの TODO に関する Issue #8 を作成。
