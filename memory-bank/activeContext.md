...
- **プロジェクト全体の整理:** 不要なファイル、コード、コメントを削除し、メンテナンス性を向上させる。

## 2. 最近の主な変更点 (Recent Changes)

- **2025-04-11 (最新):**
  - **プロジェクト整理 (refactor/project-cleanup ブランチ):**
    - 不要なレガシーファイル `src/core/common.ts` を削除。
    - コメントアウトされていた不要な `console.warn` を `src/ui/components/notification.ts` および `src/core/common/parsers.ts` から削除。
    - `src/core/common/parsers.ts` 内の TODO コメント（スキップ行のユーザー通知）について GitHub Issue #8 を作成。
...
1. **プロジェクト整理内容のコミット:** `refactor/project-cleanup` ブランチで行った変更をコミットする。
2. **Pull Request 作成:** `refactor/project-cleanup` ブランチから `main` ブランチへの Pull Request を作成し、レビューを依頼する。
3. **CI/CD ワークフローテスト:** (PR マージ後 or 別途実施)
   - **CI:** `main` ブランチへのマージ時に `ci.yml` ワークフローが成功することを確認する。
