 ...
 * @param {number} level - 詳細度レベル（1-3）
 * @param {string} type - メッセージの種類（'info', 'success', 'error', 'warning'）
 */
function logTestResult(message, level = 1, type = 'info') {
 ...
 * @param {string} filePath - テスト対象ファイルのパス
 * @returns {Promise<Object[]>} - パースされた症例データ
 */
...
/**
 * 症例データの統合をテストする関数
 * @param {Object[]} cases1 - 1つ目の症例データ配列
 * @param {Object[]} cases2 - 2つ目の症例データ配列
 * @returns {Object[]} - 統合された症例データ
 */
function testMergeCases(cases1, cases2) {
...
/**
 * 短手３該当症例の判定をテストする関数
 * @param {Object[]} mergedCases - 統合された症例データ
 * @returns {Object[]} - 短手３に該当する症例データ
 */
function testEvaluateCases(mergedCases) {
...
/**
 * 結果のフォーマットをテストする関数
 * @param {Object[]} resultCases - 短手３に該当する症例データ
 * @param {string} headerLine - ヘッダー行
 * @returns {string} - フォーマットされた結果テキスト
 */
function testFormatResults(resultCases, headerLine) {
 ...
 * @param {string} outputText - 実際の出力テキスト
 * @param {string} expectedContent - 期待される結果テキスト
 * @returns {boolean} - 比較結果（一致する場合はtrue）
 */
function testCompareResults(outputText, expectedContent) {
...
async function runAllTests() {
    try {
        ...
        logTestResult(`テスト実行中にエラーが発生しました: ${error.message}`, 1, 'error');
        if (error.stack) {
