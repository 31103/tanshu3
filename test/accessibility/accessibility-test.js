...
async function testContrastRatio(page) {
    ...
    const textElements = await page.evaluate(() => {
        const elements = Array.from(document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, span, a, button, label, input, select, textarea'));
...
async function testSemanticHTML(page) {
    ...
    const htmlStructure = await page.evaluate(() => {
        function getElementStructure(element, depth = 0) {
            ...
            const children = Array.from(element.children).map(child => getElementStructure(child, depth + 1));
    ...
    const semanticResults = {
        ...
        hasHeadings: await page.evaluate(() => document.querySelector('h1, h2, h3, h4, h5, h6') !== null),
        hasLandmarks: await page.evaluate(() => document.querySelector('[role="banner"], [role="navigation"], [role="main"], [role="contentinfo"]') !== null)
    };
